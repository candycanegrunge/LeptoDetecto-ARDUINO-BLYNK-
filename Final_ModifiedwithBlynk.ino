...
void loop()
{
 ...
 ConVal( moisture, tdsValue );// Call Convert Value function

 CondScore( tempCelsius, phValue, moisture_percentage, h, salinitypercent); // Call Condition Score function

 Token2Score(token); // Call function for deciding condition score

 Carry(tempCelsius, phValue, moisture_percentage, h, salinitypercent, score); // Call Carry value for link generate function
 ...
 void TDS_Sensor()
{
 ...
 void ConVal(float moisture, float tdsValue) {

  moisture_percentage = ( 100 - ( (moisture/960.00) * 100)); //convert raw data into percentage value
  salinitypercent = tdsValue/10000;

}


void CondScore(float tempCelsius, float phValue, float moisture_percentage, float h, float salinitypercent){

  //Determine pH condition token
  if( (phValue > 7.0) && (phValue < 7.4)){
    token = token + hazard;
  }
  else if((phValue > 7.2) && (phValue < 8.0)){
    token = token + medium;
  }
  ...
  //Determine Temperature condition token
  if( (tempCelsius > 28) && (tempCelsius < 30)){
    token = token + hazard;
  } //redundancy on hazard and medium because both has same range
  else if((tempCelsius > 28) && (tempCelsius < 30)){
    token = token + medium;
  }
  else if((tempCelsius > 38) && (tempCelsius < 28)){
    token = token + safe;
  }
  else{
    token = token + safe;
  ...
  //Determine TDS condition token
  if( salinitypercent <= 3){
    token = token + hazard;
  }
  else if( salinitypercent >= 3){
    token = token + medium;
  } //redundancy on medium and safe because both has same range
  else if( salinitypercent >= 3){
    token = token + safe;
  }
  else{
  ...
  //Determine Soil Moisture condition token
  ...
  //Determine Humidity condition token
  if( h > 65){
    token = token + hazard;
  }
  else if( h > 65){
...
void Token2Score(float token){

  if( token == 500 || token == 410 || token == 401 || token == 320 || token == 311 || token == 302){
    score = 3;
  }
  else if( token == 50 || token == 41 || token == 140 || token == 230 || token == 131 || token == 32){
    score = 2;
  }
  else if( token == 5 || token == 14 || token == 104 || token == 203 || token == 23 || token == 113){
    score = 1;
  }
  else if( token == 212 || token == 221 || token == 122){
    score = 2;
  }
 ...
 void Carry(float tempCelsius, float phValue, float moisture_percentage, float h, float salinitypercent, float score){

  String req = "https://128.199.144.129/external/api/batch/update?token=J1C2UI0hOZCuVkU83fpsqJQpKYyTI91w"; // http + token 
  req = req + "&V0=";
  req = req + phValue;
  req = req + "&V1=";
  req = req + tempCelsius;
  req = req + "&V2=";
  req = req + salinitypercent;
  req = req + "&V3=";
  req = req + moisture_percentage;
  req = req + "&V4=";
  req = req + h;
  req = req + "&V5=";
  req = req + score;
